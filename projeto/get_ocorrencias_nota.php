...
foreach ($ocorrencias as $obj) {
    ...
    $hidden_descricao = "<input type='hidden' id='" .$obj->id."_descricao' value='". htmlspecialchars($obj->descricao, ENT_QUOTES) ."'>";
    $btn_alterar = $hidden_descricao."<button title='Excluir' class='btn btn-primary btn-sm' type='button' onclick='alterar(" . $obj->id . "," . $usuario_logado->id . ",\"" . htmlspecialchars(addslashes($obj->descricao), ENT_QUOTES) . "\"," . $prestador->id . "," . $id_nota . ")'><i class='fas fa-edit'></i></button>";
    $btn_resolver = "&nbsp;&nbsp;" . $txt_status;
    if ($obj->resolvido == 0) {
        echo "  <td align='center'>" . $btn_alterar . "&nbsp;&nbsp;<button title='Excluir' class='btn btn-danger btn-sm' type='button' onclick='excluir(" . $obj->id . "," . $usuario_logado->id . ",\"" . htmlspecialchars(addslashes($obj->descricao), ENT_QUOTES) . "\"," . $prestador->id . "," . $id_nota . ")'><i class='far fa-trash-alt'></i></button>" . $btn_resolver . "</td>";
    } else {
