...
//            echo "  <td>".($obj->responsavel > 0 ? $manterUsuario->getUsuarioPorId($obj->responsavel)->nome:'')."</td>";
    ...
    echo "<a href='painel_etapas.php?tarefa=" . $obj->id . "' class='btn btn-dark btn-sm' title='Painel de Etapas' target='_blank'><i class='fa fa-signal'></i></a>&nbsp;&nbsp;";
    ...
    if ($obj->excluir) {
        ...
        if ($editar) {
            echo $hidden_descricao."&nbsp;&nbsp;<button title='Alterar tarefa' class='btn btn-primary btn-sm' type='button' onclick='alterar(" . $obj->id . ",\"" . $obj->nome . "\",\"" . $obj->descricao . "\",\"" . $obj->categoria . "\",\"" . $obj->inicio . "\",\"" . $obj->fim . "\",\"" . $obj->tipo . "\"," . $obj->criador . "," . $obj->responsavel . "," . $obj->equipe . ")'><i class='fas fa-edit'></i></button>&nbsp;&nbsp;<button title='Excluir tarefa' class='btn btn-danger btn-sm' type='button' onclick='excluir(" . $obj->id . ",\"" . $obj->nome . "\")'><i class='far fa-trash-alt'></i></button>";
            ;
        }
    } else {
        echo "  <a href='gerenciar_etapas.php?tarefa=" . $obj->id . "' class='btn btn-warning btn-sm' title='Gerenciar etapas'><i class='fa fa-bars'></i></a>";
        ...
        if ($editar) {
            echo $hidden_descricao." &nbsp;&nbsp;<button title='Alterar tarefa' class='btn btn-primary btn-sm' type='button' onclick='alterar(" . $obj->id . ",\"" . $obj->nome . "\",\"" . $obj->descricao . "\",\"" . $obj->categoria . "\",\"" . $obj->inicio . "\",\"" . $obj->fim . "\",\"" . $obj->tipo . "\"," . $obj->criador . "," . $obj->responsavel . "," . $obj->equipe . ")'><i class='fas fa-edit'></i></button>&nbsp;&nbsp;<button title='Excluir tarefa' class='btn btn-secondary btn-sm' type='button' title='Possuí dependências!'><i class='far fa-trash-alt' alt='Possuí dependências!'></i></button>";
        }
    }
    echo "</td>";
    echo "</tr>";
