...
class ManterPrestador extends Model {
    ...
    function getPrestadoresMaioresValoresPorCompetencia($competencia = "", $limit = 5) {
        $sql = "SELECT p.razao_social, np.valor FROM prestador p, fiscal_prestador fp, pagamento pg, nota_pagamento np, tipo_prestador as tp WHERE p.id_tipo_prestador = tp.id AND tp.id != '12' AND p.id = fp.id_prestador AND fp.id = pg.id_fiscal_prestador AND pg.id = np.id_pagamento AND pg.competencia = '". $competencia ."'
    ...
    function getMaioresValoresNotaPagamentoPorCompetenciaAdm($ano = "2024") {
        $sql = "SELECT DISTINCT pg.competencia, SUM(CAST(REPLACE(REGEXP_REPLACE(np.valor, '[^0-9,]', ''), ',', '.') AS DECIMAL(15,2))) AS valor_real, ROUND(SUM(CAST(REPLACE(REGEXP_REPLACE(np.valor, '[^0-9,]', ''), ',', '.') AS DECIMAL(15,2))), 0) AS valor_para_dashboard 
        FROM prestador p, fiscal_prestador fp, pagamento pg, nota_pagamento np, tipo_prestador as tp WHERE p.id_tipo_prestador = tp.id AND tp.id = '12' AND p.id = fp.id_prestador AND fp.id = pg.id_fiscal_prestador AND pg.id = np.id_pagamento AND np.data_pagamento IS NOT NULL AND TRIM(pg.competencia) LIKE '%" . $ano . "'
        GROUP BY pg.competencia ORDER BY competencia  ASC";
        $resultado = $this->db->Execute($sql);
    ...
    function getNotaPagamentoTodasCompetenciasAdm($anos_competencia){
        $prestadores = $this->getMaioresValoresNotaPagamentoPorCompetenciaAdm($anos_competencia);
        $lista_final = [];
        $soma_anual = 0;
    ...
    function getDadosPrestadoresNotaPagamentoAdm($quantidade_exibicao, $competencia){
        $total = $this->getMaioresValoresNotaPagamentoPorCompetenciaAdm($competencia)[0]->valor_real;
        $dados = [
