...
//        exit;
    ...
    function getUsuariosPorIdSetor(int $id_setor)
    {
        $sql = "select u.id,u.nome, u.sexo,u.login,u.matricula,u.cargo,u.email,u.nascimento, u.whatsapp, u.linkedin,u.ativo,u.agenda,u.id_setor,u.aniversariantes, u.codigo_lotacao, u.descricao_lotacao, u.simbolo_cargo, u.cargo_efetivo, u.cpf, u.horario FROM usuario as u where id_setor =" . $id_setor;
        $resultado = $this->db->Execute($sql);
        $array_dados = array();
        while ($registro = $resultado->fetchRow()) {
            $dados = new Usuario();
